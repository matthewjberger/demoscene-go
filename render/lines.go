...
func (l *Lines) AddBox(bounds BoundingVolume, model *transform.Mat4, color [4]float32) {
	...
	for i := 0; i < len(BoundingBoxEdges); i += 2 {
		a := worldCorners[BoundingBoxEdges[i]]
		b := worldCorners[BoundingBoxEdges[i+1]]
...
func NewLinesPass(device *wgpu.Device, surfaceFormat wgpu.TextureFormat, aspect func() float32) (*Pass, error) {
...
func linesPrepare(s any, context *PassContext) error {
	...
	viewProj := CameraViewProjection(camera, state.aspectFn())
...
func linesExecute(s any, context *PassContext) error {
	state := s.(*linesPassState)
	if state.count == 0 {
...
func AddLinesPass(renderer *Renderer) (*Pass, error) {
